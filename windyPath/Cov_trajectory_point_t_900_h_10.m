function suipian = Cov_trajectory_point_t_900_h_10()
% clc;
% close all;
% clear;
%初始值设定
...
%             suipian1=[suipian1;x(1),x(2)];
            suipian_t=[suipian_t;x(1),x(2)];
        end
    end
...
%     xx([Index],:)=[];
     ...
     %% 对xx数组去除含有相同坐标信息的点
suipian_t=roundn(suipian_t,-4);
Index=[];
q=1;
    for i=1:Point^2
        for j=(i+1):Point^2
            if suipian_t(i,:)==suipian_t(j,:)
                Index(q)=j;
                q=q+1;
            end
        end
...
%     hold on;
    ...
    %% 二维最后一个时刻凸多边型边界显示 
     figure(13);
    %     [k area]= convhull(xx(:,1),xx(:,2));
    [k area]= convhull(suipian_t(:,1),suipian_t(:,2));
    hold on;
    xlabel('x/km');
    ylabel('y/km');
    plot(suipian_t(:,1),suipian_t(:,2), '.', 'markersize',10,'HandleVisibility','off');
    plot(suipian_t(k,1), suipian_t(k,2),'-','LineWidth',1.5,'HandleVisibility','off'); % 逆时针连接最外层边界点
%     legend('初始边界');
    set(gca, 'XLim',[min(suipian_t(k,1))-60 max(suipian_t(k,1))+60]); % X轴的数据显示范围
    set(gca, 'YLim',[min(suipian_t(k,2))-10 max(suipian_t(k,2))+10]);
    grid on;
    hold on;
    %% 计算路径距离
%     %下路径
...
%    suipian=[inX,inY];
   ...
   % 这里面k值影响expand函数里减去最后一点
   hold on;
   p=[suipian_t(k,1), suipian_t(k,2)]; 
   ii=size(k,1);% k的行数
   [newp2]=expand(p,ii);
   plot(newp2(:,1),newp2(:,2),'k-','LineWidth',1.5,'HandleVisibility','off');
   set(gca, 'XLim',[min(p(:,1))-80 max(p(:,1))+80]); % X轴的数据显示范围
   set(gca, 'YLim',[min(p(:,2))-20 max(suipian_t(:,2))+20]);
   
%    fill(newp2(:,1),newp2(:,2),[0.5 0.5 0.5]);
%    fill(suipian_t(k,1), suipian_t(k,2),'k');
   %得到扩展区域所有点的坐标
   xVector=100:1:450;
   yVector=-40:1:20;
   ...
   in=inpolygon(x,y,newp2(:,1),newp2(:,2));
%    in=inpolygon(x,y,suipian_t(:,1),suipian_t(:,2));
   inX=x(in);
   inY=y(in);
   suipian=[inX,inY];
